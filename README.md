 ...
 └─ normalize (Unicode NFKC, strip zero-width/tag chars, single HTML/URL decode,
     ...
     └─ FastText heads (always run, fast first-pass routing)
         ...
         └─ Router decides which transformer confirmers to run
             · devndeploy/bert-prompt-injection-detector       (prompt injection)
             · madhurjindal/Jailbreak-Detector                 (jailbreak)
             · ifmain/ModerationBERT-En-02                     (broad moderation)
...
| Role | Model | License |
|---|---|---|
| Prompt injection | `devndeploy/bert-prompt-injection-detector` | see model card |
| Jailbreak | `madhurjindal/Jailbreak-Detector` | MIT |
| Broad moderation (default) | `ifmain/ModerationBERT-En-02` | see model card |
...
unavailable, a label underrepresented, an optional model export failed but the fallback
...
- `ifmain/ModerationBERT-En-02` is the default broad-moderation backend. It
  exposes 18 independent moderation categories, so the wrapper scores it with
  sigmoid probabilities rather than softmax.
